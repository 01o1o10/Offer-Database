        ...
        sql.query("select * from cuprumprices where cp_date='" + this.getDateNow() + "';", function(data){
            if(data.length == 0){
                request('https://www.lme.com/Metals/Non-ferrous/Copper#tabIndex=0', function (error, response, body) {
                    if(error){
                        var html = 'Cuprum price can not get from the page!</br><form><div class="form-group"><alert type="text" class="form-control" id="cuprum-price-input" placeholder="Type cuprum today price..."></div><div class="form-group"><button type="button" class="form-control btn btn-primary" id="cuprum-price-submit">Set</button></div></form>'
                        ui.setAlertModal(html , false)
                        throw error
                    }
                    else{
                        var html = document.createElement('DIV')
                        html.innerHTML = body
                        var price = html.getElementsByTagName('table')[0].rows[1].cells[2].textContent
        ...
        sql.query("select * from leadprices where lp_date='" + this.getDateNow() + "';", function(data){
            if(data.length == 0){
                request('https://www.lme.com/Metals/Non-ferrous/Lead#tabIndex=0', function (error, response, body) {
                    if(error){
                        var html = 'Lead price can not get from the page!</br><form><div class="form-group"><input type="text" class="form-control" id="alert-price-input" placeholder="Type lead today price..."></div><div class="form-group"><button type="button" class="form-control btn btn-primary" id="lead-price-submit">Set</button></div></form>'
                        ui.setAlertModal(html , false)
                        throw error
                    }
                    else{
        ...
        sql.query("select * from zincprices where zp_date='" + this.getDateNow() + "';", function(data){
            if(data.length == 0){
                request('https://www.lme.com/Metals/Non-ferrous/Zinc#tabIndex=0', function (error, response, body) {
                    if(error){
                        var html = 'Zinc price can not get from the page!</br><form><div class="form-group"><input type="text" class="form-control" id="alert-price-input" placeholder="Type zinc today price..."></div><div class="form-group"><button type="button" class="form-control btn btn-primary" id="zinc-price-submit">Set</button></div></form>'
                        ui.setAlertModal(html , false)
                        throw error
                    }
                    else{
